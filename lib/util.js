...
function getPaths(publicPath, compiler, url) {
  ...
  // all this is to find `publicPath` and `outputPath`
  // in both single- and multi-compilers modes
...
// call fn(context.webpackStats) if valid
// otherwise queue `fn` to be called until `context.state` is valid (true)
// ^ which means "done" is called
...
module.exports = {
  // see middleware.js re: comment what this function does
  ...
  handleRangeHeaders(content, req, res) {
    ...
    if (req.headers.range) {
      // `req.headers.range` tells where to continue
      ...
      // regular response)
      // ---
      // only care when `ranges.length === 1`
      if (ranges !== -2 && ranges.length === 1) {
        const { length } = content;

        // Content-Range
        res.statusCode = 206;
        // `Content-Range` indicates where a partial message belongs
        // in a full body message
        res.setHeader(
          'Content-Range',
